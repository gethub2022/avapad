...
describe('CurrencyAmount', () => {
  ...
  describe('#ether', () => {
    it('produces ether amount', () => {
      const amount = CurrencyAmount.fromRawAmount(Ether.onChain(3), 100)
      expect(amount.quotient).toEqual(JSBI.BigInt(100))
      expect(amount.currency).toEqual(Ether.onChain(3))
    })
  })

  it('token amount can be max uint256', () => {
    const amount = CurrencyAmount.fromRawAmount(new Token(3, ADDRESS_ONE, 18), MaxUint256)
    expect(amount.quotient).toEqual(MaxUint256)
  })
  it('token amount cannot exceed max uint256', () => {
    expect(() =>
      CurrencyAmount.fromRawAmount(new Token(3, ADDRESS_ONE, 18), JSBI.add(MaxUint256, JSBI.BigInt(1)))
    ).toThrow('AMOUNT')
  })
  it('token amount quotient cannot exceed max uint256', () => {
    expect(() =>
      CurrencyAmount.fromFractionalAmount(
        new Token(3, ADDRESS_ONE, 18),
  ...
  describe('#toFixed', () => {
    it('throws for decimals > currency.decimals', () => {
      const token = new Token(3, ADDRESS_ONE, 0)
  ...
  describe('#serialize', () => {
    it('renders string with address and amount for Token', () => {
      const token = new Token(3, ADDRESS_ONE, 3, 'MOONBEAM')
      const amount = CurrencyAmount.fromRawAmount(token, 123456)
